...
x = Embedding(output_dim=256, input_dim=n_words, input_length=sequence_length)(sequence_input)
...
sequence_output = Dense(1, activation='sigmoid', name='sequence_output')(lstm_out)

#This defines a model with two inputs and two outputs:
model = Model(input=sequence_input, output=sequence_output)

myOpt = keras.optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=None, decay=0.0)
#myOpt = keras.optimizers.Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
...
model.compile(optimizer=myOpt,
              loss={'sequence_output': 'binary_crossentropy'},
              loss_weights={'sequence_output': 1.})
# And trained it via:
model.fit({'sequence_input': x_sequence_train},
          {'sequence_output': y_sequence_train},
          epochs=40, batch_size=100,validation_data=([x_sequence_valid],[y_sequence_valid]))

result = model.evaluate(x_sequence_test,y_sequence_test)
#Print the results
for i in range(len(model.metrics_names)):
    print("Metric ",model.metrics_names[i],":",str(round(result,2)))

preds=np.squeeze(np.round(model.predict(x_sequence_test)))
print('Test Acc.: %.3f' % (np.sum(preds == y_sequence_test) / len(y_sequence_test)))

# from sklearn.metrics import mean_squared_error, mean_absolute_error
# print("MSE :",mean_squared_error(y_sequence_test["Actual Dose Compliance"].values,y_sequence_test["Predicted Dose Compliance"].values))
# print("MAE :",mean_absolute_error(y_sequence_test["Actual Dose Compliance"].values,y_sequence_test["Predicted Dose Compliance"].values))
