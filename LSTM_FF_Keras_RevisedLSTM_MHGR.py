...
# And trained it via:
model.fit({'sequence_input': x_sequence_train, 'static_input': x_static_train},
          {'sequence_output': y_sequence_train, 'static_output': y_sequence_train},
          epochs=40, batch_size=100,validation_data=([x_sequence_valid,x_static_valid],[y_sequence_valid,y_sequence_valid]))

result = model.evaluate([x_sequence_test,x_static_test],[y_sequence_test,y_sequence_test])
#Print the results
for i in range(len(model.metrics_names)):
    print("Metric ",model.metrics_names[i],":",str(round(result[i],2)))

preds=np.squeeze(np.round(model.predict([x_sequence_test,x_static_test])))
#get the best
print('Test Acc.: %.3f' % (max(np.sum(preds[0,] == y_sequence_test),np.sum(preds[1,] == y_sequence_test)) / len(y_sequence_test)))
